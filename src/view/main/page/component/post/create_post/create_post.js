...
function Create_post({ emailS, codeS, this_user_id, route_params }) {
  ...
  function handleImage1() {
    const newImageUri = "file:///" + images[0].uri.split("file:/").join("");
  ...
  async function handleImage2() {
    for (var i = 0; i < images.length; i++) {
      var newImageUri =
        "file:///" + images[i].uri.split("file:/").join("");
      const payload = new FormData();
      payload.append("image", {
        uri: newImageUri,
        type: mime.getType(newImageUri),
      ...
      <ScrollView pt="2" bgColor="white">
        <Formik
          ...
          onSubmit={async (values, actions) => {
            ...
            if (parseInt(route_params?.edit) === 1) {
              link_post =
                link.server_link +
