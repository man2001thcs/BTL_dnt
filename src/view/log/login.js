...
function Login({ codeChange, emailChange, setLogin, logined, setInfo }) {
  ...
  const SignupSchema = Yup.object().shape({
    emailS: Yup.string().email("Email không hợp lệ").required("Cần điền email!"),
    passwordS: Yup.string()
      .min(2, "Mật khẩu cần dài hơn!")
            ...
            await fetch(
              ...
              .then((data) => {
                ...
                if (data?.code === "ACCOUNT_LOGIN_OK") {
                  ...
                  if (parseInt(JSON.parse(data?.data).new) === 1) {
                    setInfo(JSON.parse(data?.data));
                    navigation.navigate("Tutorial_page");
                  } else {
                    setInfo(JSON.parse(data?.data));
                    setLogin(true);
                  }
