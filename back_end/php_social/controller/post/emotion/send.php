...
if ($contentType === "application/json") {
    ...
    if (is_array($decoded)) {
        ...
        if (strcmp($user->login_code($decoded['emailS']), $decoded['codeS']) == 0 && $user->is_admin($decoded['emailS']) == false) {
            ...
            $emotion_post_list_data = $emotion_post_list->findByPostId($decoded['id']);
            ...
            if ($key == 'none') {
                ...
                $data_emotion_sub = array(
                    'EmotionPostList' => array(
                        'id' => $emotion_post_list_data['EmotionPostList']['id'],
                        'post_id' => $post_id,
                ...
                $dataSub_notify = $notification->get_notify_single_post($decoded['emailS'], $post_data['Post']['user_account'], $post_id);
                ...
                $dataSub_notify['Notification']['type'] = 10 + intval($decoded['emotionState']);
                ...
                if ($emotion_post_list->save($data_emotion_sub)) {
                    echo json_encode($data_emotion_sub);
