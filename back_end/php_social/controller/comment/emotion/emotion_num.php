...
if ($contentType === "application/json") {
    ...
    if (is_array($decoded)) {
        ...
        if (strcmp($user->login_code($decoded['emailS']), $decoded['codeS']) == 0 && $user->is_admin($decoded['emailS']) == false) {
            //echo("id: " . $decoded['id']);
            $emo_list_detail = $emotion_comment_list->findByCommentId($decoded['comment_id']);
            //echo ("id: " . json_encode($emo_list_detail));
            $emotion_data_string_array = $emo_list_detail['EmotionCommentList'];
            ...
            $emotion_comment_list->sendResponse(200, json_encode($response));
        } else {
